...
function popularis_customizer_theme_info($wp_customize) {
    ...
    $theme_info .= sprintf(__('You can use this theme to create a website like these %1$s', 'popularis'), '<a href="' . esc_url('https://themes4wp.com/theme/popularis/') . '" target="_blank">' . esc_html__('demos', 'popularis') . '</a>');

    $theme_info .= '</p><p>';
    /* translators: %s: "documentation" string */
    $theme_info .= sprintf(__('For step-by-step videos and text tutorials, see %1$s', 'popularis'), '<a href="' . esc_url('http://demo.themes4wp.com/documentation/category/popularis/') . '" target="_blank">' . esc_html__('documentation', 'popularis') . '</a>');
    $theme_info .= '</p>';
...
function popularis_page_sections_pro($manager) {
    ...
    $manager->add_section(
            new popularis_Customize_Section_Pro(
                ...
                'pro_url' => 'https://themes4wp.com/product/popularis-pro/'
                    )
            )
    );
}
